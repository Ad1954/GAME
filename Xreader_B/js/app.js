...
function setupMediaSessionHandlers() {
  if ('mediaSession' in navigator) {
    ...
    navigator.mediaSession.setActionHandler('previoustrack', () => {
      console.log('Bluetooth / MediaSession: Prev track clicked');
      if (speechEngine === 'native') {
        ...
        // Background Audiobook Mode: Skip directly to previous chapter starting from sentence 0
        if (currentChapterIndex > 0) {
          const prevChapIdx = currentChapterIndex - 1;
          loadChapter(currentBook.id, prevChapIdx, 0).then(() => {
    ...
    navigator.mediaSession.setActionHandler('nexttrack', () => {
      console.log('Bluetooth / MediaSession: Next track clicked');
      if (speechEngine === 'native') {
        if (currentSentenceIndex < flatSentences.length - 1) {
          jumpToSentence(currentSentenceIndex + 1);
          if (isPlaying) playSentence(currentSentenceIndex);
        ...
        // Background Audiobook Mode: Skip directly to next chapter starting from sentence 0
        if (currentChapterIndex < currentChapters.length - 1) {
          const nextChapIdx = currentChapterIndex + 1;
          loadChapter(currentBook.id, nextChapIdx, 0).then(() => {
            if (isPlaying) playSentence(0);
  ...
  play(index, isNatural = false) {
    ...
    playTimeoutId = setTimeout(() => {
      ...
      currentUtterance.onend = () => {
        if (isUserChangingState) return;

        if (isPlaying) {
          setTimeout(() => {
            if (isPlaying && !isUserChangingState) advanceToNextSentence();
          }, 80);
        }
      };
      ...
      // Release change state lock after speaking starts
      isUserChangingState = false;
    }, 100);
  },
...
// ==========================================
// Module B: Background Audiobook Player (Combined SpeechSynthesis - Offline)
// ==========================================
const BackgroundPlayer = {
  play(index, isNatural = false) {
    if (playTimeoutId) clearTimeout(playTimeoutId);
    
    // Stop highlighting player
    ...
    // iOS Safari Queue Freeze Workaround:
    playTimeoutId = setTimeout(() => {
      // Compile remaining text of current chapter starting from selected sentence
      ...
      currentUtterance.onend = () => {
        ...
        // If sleep timer is set to chapter end, trigger sleep
        if (sleepChapterEndMode) {
          console.log('Sleep Timer: End of chapter reached. Stopping.');
          sleepChapterEndMode = false;
          DOMElements.settingSleepTimer.value = 'off';
          DOMElements.sleepCountdown.style.display = 'none';
          resetPlaybackState();
          return;
        }

        if (isPlaying) {
          // Chapter finished naturally! Advance to next chapter
          setTimeout(() => {
            if (isPlaying && !isUserChangingState) {
              const nextChapterIdx = currentChapterIndex + 1;
              if (nextChapterIdx < currentChapters.length) {
                console.log('BackgroundPlayer: Chapter ended. Moving to next chapter.');
                currentChapterIndex = nextChapterIdx;
                const nextChapter = currentChapters[nextChapterIdx];
                ...
                // Load UI DOM asynchronously
                loadChapter(currentBook.id, nextChapterIdx, 0);

                // Play first sentence of new chapter with 100ms safety queue flush delay
                setTimeout(() => {
                  if (isPlaying && !isUserChangingState) {
                    playSentence(0, true);
                  }
      ...
      currentUtterance.onerror = (e) => {
        console.warn('BackgroundPlayer Speech Error:', e);
        if (isUserChangingState) return;

        if (e.error !== 'interrupted' && isPlaying) {
          setTimeout(() => {
            if (isPlaying && !isUserChangingState) {
              // Try to skip to next chapter on synthesis error
              const nextChapterIdx = currentChapterIndex + 1;
              if (nextChapterIdx < currentChapters.length) {
                currentChapterIndex = nextChapterIdx;
...
function startAudioKeepAlive() {
  ...
  const activePlat = platform === 'auto' ? detectedPlatform : platform;
  if (activePlat === 'PC') {
    console.log('PC platform detected. Skipping background keep-alive oscillators.');
    return;
  }
...
// --- Sleep Timer (睡眠定時器) ---
function startSleepTimer(minutesVal) {
  if (sleepTimerId) {
  ...
  if (minutesVal === 'chapter') {
    sleepChapterEndMode = true;
    DOMElements.sleepCountdown.textContent = '⏳ 唸完本章停止';
    DOMElements.sleepCountdown.style.display = 'inline-block';
    return;
  }

  const minutes = parseInt(minutesVal);
  if (isNaN(minutes)) return;

  sleepTimeRemaining = minutes * 60;
  updateSleepCountdownUI();
...
function updateSleepCountdownUI() {
  ...
  DOMElements.sleepCountdown.textContent = `⏳ ${formattedTime}`;
  DOMElements.sleepCountdown.style.display = 'inline-block';
}

function triggerSleepStop() {
  console.log('Sleep Timer expired. Gracefully stopping playback.');
  DOMElements.settingSleepTimer.value = 'off';
  DOMElements.sleepCountdown.style.display = 'none';
  resetPlaybackState();
}
...
function setupSettingsDrawerEvents() {
  ...
  DOMElements.settingSleepTimer.addEventListener('change', (e) => {
    startSleepTimer(e.target.value);
  });

  // Handle clicking sleep countdown text to cancel timer
  DOMElements.sleepCountdown.addEventListener('click', () => {
    if (confirm('是否取消睡眠定時器？')) {
      DOMElements.settingSleepTimer.value = 'off';
      startSleepTimer('off');
    }
  });
